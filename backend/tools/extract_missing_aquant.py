...
IMG_ROOT = os.path.join("static", "images", "aquant")
PDF_PATH = os.path.join("uploads", "Aquant Price List Vol 15. Feb 2026_Searchable.pdf")
...
def main():
    ...
    # Identify missing Aquant items
    missing_items = []
    for item in items:
        brand = str(item.get("brand", "")).lower()
        if "aquant" not in brand:
            continue
        
        imgs = item.get("images") or []
        if not imgs:
            # If no image assigned at all, add to missing
    ...
    print(f"Found {len(missing_items)} missing Aquant items.")
    ...
    for page_num, p_items in by_page.items():
        ...
        for item in p_items:
            ...
            if not text_rects:
                # Fallback: Just search for the first part
                base = code.split()[0]
            ...
            if text_rects:
                ...
                for img_info in extracted_images:
                    ...
                    if dist < min_dist and img_info["area"] > 500: # Smaller threshold for Aquant
                        min_dist = dist
                        best_img = img_info
            ...
            if best_img:
                filename = f"AQ_{safe_code}_{page_num}.{best_img['ext']}"
                local_path = os.path.join(IMG_ROOT, filename)
                with open(local_path, "wb") as f:
                    f.write(best_img["bytes"])
                    
                public_path = f"/static/images/aquant/{filename}"
                item["images"] = [public_path]
                fixed += 1
                print(f"Extracted: {code} (p{page_num}) -> {filename}")

    # Save index
    with open(INDEX_PATH, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, separators=(",", ":"))
        
    print(f"\nSuccessfully extracted and assigned {fixed} images from Aquant PDF.")
