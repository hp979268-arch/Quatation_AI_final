        ...
        for b in blocks:
            ...
            if "trim + valve" in text or "trim & valve" in text:
                ...
                if match:
                    ...
                    # Only do 22786 series to save time
                    if "22786" not in code:
                        continue
...
print(f"Extracted {len(extracted)} exact box images to _FINAL.png")
...
for code, filename in extracted:
    for item in db["stored_items"]:
        if code == item.get("search_code", ""):
            ...
            print(f"Updated index array for {code} to {filename}")

with open(index_file, "w", encoding="utf-8") as f:
    json.dump(db, f)

print("Updated search_index_v2.json with _FINAL.png paths!")
