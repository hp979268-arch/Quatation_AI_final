...
print(f"Extracted {len(extracted)} exact box images to _v5.png")
...
for code, filename in extracted:
    for item in db["stored_items"]:
        if code in item.get("search_code", ""):
            # Overwrite images with just this one
            item["images"] = ["/static/images/Kohler/" + filename]

with open(index_file, "w", encoding="utf-8") as f:
    json.dump(db, f)

print("Updated search_index_v2.json with _v5.png paths!")
